...
export default function LadimusReview() {
  return (
    <>
      <Section aria-labelledby="review-title" className={styles.hero}>
        <a className={styles.back} href="/work">← All work</a>
        ...
        <dl className={styles.facts}><div><dt>Engineer</dt><dd>{profile.name}</dd></div><div><dt>Context</dt><dd>Independent Ladimus work</dd></div><div><dt>Focus</dt><dd>Validation-oriented AI engineering</dd></div></dl>
      </Section>
      ...
      <Section aria-labelledby="workflow-title" className={styles.band}>
        <div className={styles.sectionHead}><p className={styles.eyebrow}>04 / Workflow perspective</p><h2 id="workflow-title">From output to an informed decision.</h2><p>A conceptual review process—not a claim about a shipped implementation.</p></div>
        <ol className={styles.workflow}>{reviewStory.workflow.map((step, index) => <li key={step.title}><span aria-hidden="true">0{index + 1}</span><div><h3>{step.title}</h3><p>{step.copy}</p></div></li>)}</ol>
      </Section>
